...
async function fetchHtmlResponse(
  ...
  try {
    for (
      ...
      const response = await fetch(currentUrl.toString(), {
        headers: {
          ...
          "User-Agent": "ChefnessRecipeImporter/1.0 (+https://chefness.app)",
        },
        redirect: "manual",
      ...
      if (redirectCount === MAX_REDIRECTS) {
        throw new FetchError(
          508,
          "TOO_MANY_REDIRECTS",
          "Recipe site redirected too many times.",
        );
      }
    ...
    throw new FetchError(
      ...
      aborted
        ? "Recipe site took too long to respond."
        : "Could not fetch recipe site.",
    );
  } finally {
...
function noJsonLdRecipe(): Response {
  return errorJson(
    422,
    "NO_JSON_LD_RECIPE",
    "Site doesn't support extraction.",
  );
}
