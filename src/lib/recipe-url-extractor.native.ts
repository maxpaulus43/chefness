...
export interface ExtractRecipeFromUrlOptions { url: string; signal?: AbortSignal; }
export interface ExtractedRecipeFromUrl { recipe: CreateRecipeInput; sourceUrl: string; sourceName: string; }
type JsonLd = string | number | boolean | null | JsonLd[] | { [key: string]: JsonLd };
const MAX_HTML_LENGTH = 2_000_000;

/** Native fetch has no browser CORS restriction, so URL extraction stays fully on-device. */
export async function extractRecipeFromUrl(options: ExtractRecipeFromUrlOptions): Promise<ExtractedRecipeFromUrl> {
  let source: URL;
  try { source = new URL(options.url); } catch { throw new Error("Invalid recipe URL."); }
  if (source.protocol !== "https:" && source.protocol !== "http:") throw new Error("Invalid recipe URL.");
  ...
  if (html.length > MAX_HTML_LENGTH) throw new Error("Recipe page is too large.");
  ...
  const description = stringValue(value.description).trim() || `Imported from ${source.hostname.replace(/^www\./, "")}`;
  const ingredients = stringArray(value.recipeIngredient);
  const steps = instructionTexts(value.recipeInstructions);
  if (!title || ingredients.length === 0 || steps.length === 0) throw new Error("Site doesn't support extraction.");
...
function instructionTexts(value: JsonLd | undefined): string[] {
  ...
  if (Array.isArray(value)) return value.flatMap(instructionTexts).filter(Boolean);
  if (!isRecord(value)) return [];
  const text = stringValue(value.text as JsonLd).trim();
  return text ? [text] : instructionTexts(value.itemListElement as JsonLd | undefined);
}
