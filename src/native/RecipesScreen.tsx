...
export function RecipeListScreen({ navigation }: NativeStackScreenProps<RecipesStackParamList, "RecipeList">) {
  ...
  return <View style={nativeStyles.screen}><ScrollView contentContainerStyle={nativeStyles.scroll} keyboardShouldPersistTaps="handled">
    ...
    ><View accessible accessibilityRole="button" accessibilityLabel={`${recipe.title}. ${recipe.description}. ${recipe.ingredients.length} ingredients, ${recipe.steps.length} steps`} accessibilityHint="Opens recipe details; long press for more actions"><Card><Text style={styles.recipeTitle}>{recipe.title}</Text><Text style={nativeStyles.muted}>{recipe.description}</Text><Text style={styles.meta}>{recipe.ingredients.length} ingredients · {recipe.steps.length} steps</Text></Card></View></ListInteractionRow>)}
  </ScrollView></View>;
}
...
export function RecipeDetailScreen({ route, navigation }: NativeStackScreenProps<RecipesStackParamList, "RecipeDetail">) {
  ...
  return <View style={nativeStyles.screen}><ScrollView contentContainerStyle={nativeStyles.scroll}>
    ...
    <Card><Text accessibilityRole="header" style={nativeStyles.sectionTitle}>Edit with AI</Text><Text style={nativeStyles.muted}>Describe a change, preview the complete updated recipe, then apply it.</Text><Field accessibilityLabel="Recipe edit instructions" value={instruction} onChangeText={setInstruction} multiline placeholder="Make it vegetarian and reduce prep time…" /><Button disabled={ai.status === "generating"} label={ai.status === "generating" ? "Generating…" : "Generate Preview"} onPress={() => void ai.generateEdit(recipe, instruction)} />{ai.error && <Text accessibilityLiveRegion="assertive" style={nativeStyles.error}>{ai.error}</Text>}{ai.draftRecipe && <View style={styles.preview}>
      <Text style={styles.recipeTitle}>{ai.draftRecipe.title}</Text>
      <Text style={nativeStyles.muted}>{ai.draftRecipe.description}</Text>
      <Text accessibilityRole="header" style={nativeStyles.sectionTitle}>Ingredients</Text>
      ...
      <Text accessibilityRole="header" style={nativeStyles.sectionTitle}>Steps</Text>
    ...
    <Text accessibilityRole="header" style={nativeStyles.sectionTitle}>Ingredients</Text>{recipe.ingredients.map((item, index) => <View accessible accessibilityLabel={item} key={`${item}-${index}`} style={styles.line}><Text accessible={false} style={styles.bullet}>•</Text><Text style={styles.lineText}>{item}</Text></View>)}
    <Text accessibilityRole="header" style={nativeStyles.sectionTitle}>Steps</Text>{recipe.steps.map((item, index) => <View accessible accessibilityLabel={`Step ${index + 1}. ${item}`} key={`${item}-${index}`} style={styles.line}><Text accessible={false} style={styles.step}>{index + 1}</Text><Text style={styles.lineText}>{item}</Text></View>)}
  </ScrollView></View>;
}
...
function RecipeEditForm({ recipe, onSave, onClose }: { recipe: Recipe; onSave: (data: UpdateRecipeInput) => Promise<Recipe>; onClose: () => void }) {
  ...
  return <View style={nativeStyles.screen}><ScrollView contentContainerStyle={nativeStyles.scroll} keyboardShouldPersistTaps="handled"><Text style={nativeStyles.label}>Title</Text><Field accessibilityLabel="Title" value={title} onChangeText={setTitle} /><Text style={nativeStyles.label}>Description</Text><Field accessibilityLabel="Description" value={description} onChangeText={setDescription} multiline /><Text style={nativeStyles.label}>Ingredients (one per line)</Text><Field accessibilityLabel="Ingredients, one per line" value={ingredients} onChangeText={setIngredients} multiline /><Text style={nativeStyles.label}>Steps (one per line)</Text><Field accessibilityLabel="Steps, one per line" value={steps} onChangeText={setSteps} multiline /><Button label="Save Changes" onPress={() => void save()} /><Button label="Cancel" variant="secondary" onPress={onClose} /></ScrollView></View>;
}

function lines(value: string) { return value.split("\n").map((line) => line.trim()).filter(Boolean); }
const styles = StyleSheet.create({ recipeTitle: { fontSize: 19, color: colors.espresso, fontFamily: nativeFonts.serifBold }, meta: { color: colors.saffronDeep, fontFamily: nativeFonts.sansSemiBold }, detailTitle: { fontSize: 32, lineHeight: 40, fontFamily: nativeFonts.serifBold, color: colors.espresso }, description: { color: colors.stone700, fontSize: 17, lineHeight: 25, fontFamily: nativeFonts.sans }, line: { flexDirection: "row", gap: 10, alignItems: "flex-start" }, bullet: { color: colors.saffronDeep, fontSize: 22, fontFamily: nativeFonts.sans }, step: { minWidth: 28, minHeight: 28, borderRadius: 14, overflow: "hidden", textAlign: "center", paddingHorizontal: 6, paddingVertical: 4, backgroundColor: colors.saffronTint, color: colors.saffronDeep, fontFamily: nativeFonts.sansBold }, lineText: { flex: 1, color: colors.espresso, fontSize: 16, lineHeight: 24, fontFamily: nativeFonts.sans }, preview: { gap: 8, padding: 12, backgroundColor: colors.saffronTint, borderRadius: 12 } });
