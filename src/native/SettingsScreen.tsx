...
export function SettingsScreen() {
  ...
  return <View style={nativeStyles.screen}><ScreenHeader title="Settings" /><ScrollView contentContainerStyle={nativeStyles.scroll} keyboardShouldPersistTaps="handled">
    <Text style={nativeStyles.sectionTitle}>OpenRouter</Text><Card>{settings.isOpenRouterConnected ? <><View style={styles.connected}><Ionicons name="checkmark-circle" size={22} color={colors.success} /><Text style={styles.connectedText}>Connected</Text></View><Text style={nativeStyles.muted}>Your key is securely stored only on this device.</Text><Button label="Disconnect" variant="danger" onPress={() => Alert.alert("Disconnect OpenRouter?", "AI features will stop until you reconnect.", [{ text: "Cancel", style: "cancel" }, { text: "Disconnect", style: "destructive", onPress: () => settings.updateSettings({ openRouterOAuthKey: "" }) }])} /></> : <><Text style={nativeStyles.muted}>Connect your account to chat and use AI recipe tools. OpenRouter will show a one-time code after you create the key; copy it, close the browser, then paste it into Chefness. Chefness never displays or logs your API key.</Text><Button disabled={connecting} label={connecting ? "Connecting…" : "Connect OpenRouter"} onPress={() => void connect()} /></>}</Card>
