...
export function ModelSelectionScreen({ navigation }: NativeStackScreenProps<SettingsStackParamList, "ModelSelection">) {
  ...
  >{filtered.map((model) => {
    ...
    return <Pressable accessibilityRole="button" accessibilityState={{ selected }} key={model.id} style={[styles.model, selected && styles.selectedModel]} onPress={() => { settings.updateSettings({ llmModel: model.id }); navigation.goBack(); }}><View style={styles.modelText}><Text style={styles.modelName}>{model.name}</Text><Text style={styles.modelId}>{model.id}</Text></View>{selected && <Ionicons accessibilityLabel="Selected" name="checkmark" size={24} color={colors.saffronDeep} />}</Pressable>;
  })}</ScrollView>}</View>;
}

const styles = StyleSheet.create({ connected: { flexDirection: "row", alignItems: "center", gap: 7 }, connectedText: { color: colors.success, fontWeight: "700", fontSize: 16 }, selector: { minHeight: 48, padding: 12, borderWidth: 1, borderColor: colors.stone300, borderRadius: 12, backgroundColor: colors.white, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }, selectorText: { flex: 1, color: colors.espresso, fontSize: 15 }, addRow: { flexDirection: "row", alignItems: "center", gap: 8 }, preference: { flexDirection: "row", gap: 10, alignItems: "center", padding: 11, borderRadius: 10, backgroundColor: colors.creamDeep }, preferenceText: { flex: 1, color: colors.espresso, lineHeight: 20 }, model: { padding: 13, borderRadius: 12, backgroundColor: colors.white, flexDirection: "row", alignItems: "center", gap: 12 }, modelText: { flex: 1 }, selectedModel: { borderWidth: 2, borderColor: colors.saffron }, modelName: { color: colors.espresso, fontWeight: "700" }, modelId: { color: colors.stone500, fontSize: 12, marginTop: 3 } });
