...
export function ScreenHeader({ title, action }: { title: string; action?: ReactNode }) {
  ...
  return <View style={[styles.header, reduceTransparency && styles.opaqueSurface]}><Text accessibilityRole="header" style={styles.title}>{title}</Text>{action}</View>;
}

export function Card({ children, style }: PropsWithChildren<{ style?: ViewStyle }>) {
  const { reduceTransparency } = useAccessibilityPreferences();
  return <View style={[styles.card, reduceTransparency && styles.opaqueSurface, style]}>{children}</View>;
}
...
export function Button({ label, onPress, variant = "primary", disabled = false }: {
  ...
  return <Pressable accessibilityRole="button" accessibilityState={{ disabled }} disabled={disabled} onPress={onPress} style={({ pressed }) => [styles.button, styles[variant], (pressed || disabled) && styles.dim]}>
...
export function Empty({ title, body }: { title: string; body: string }) {
  return <View style={styles.empty}><Text accessibilityRole="header" style={styles.emptyTitle}>{title}</Text><Text style={styles.muted}>{body}</Text></View>;
}
...
export const nativeStyles = StyleSheet.create({
  ...
  label: { color: colors.espresso, fontSize: 15, fontFamily: nativeFonts.sansBold },
  muted: { color: colors.stone600, lineHeight: 21, fontFamily: nativeFonts.sans },
  error: { color: colors.danger, lineHeight: 20, fontFamily: nativeFonts.sans },
  sectionTitle: { color: colors.espresso, fontSize: 20, fontFamily: nativeFonts.serifBold, marginTop: 8 },
});
...
const styles = StyleSheet.create({
  ...
  title: { flexShrink: 1, fontSize: 27, fontFamily: nativeFonts.serifBold, color: colors.espresso },
  ...
  opaqueSurface: { backgroundColor: colors.white, borderWidth: 1, shadowOpacity: 0 },
  ...
  buttonText: { color: colors.white, fontFamily: nativeFonts.sansBold }, secondaryText: { color: colors.espresso },
  chip: { minHeight: 44, justifyContent: "center", borderRadius: 999, borderWidth: 1, borderColor: colors.stone300, paddingHorizontal: 13, paddingVertical: 9, backgroundColor: colors.white },
  chipSelected: { borderWidth: 2, borderColor: colors.saffron, backgroundColor: colors.saffronTint }, chipText: { color: colors.stone600, fontFamily: nativeFonts.sans }, chipTextSelected: { color: colors.saffronDeep, fontFamily: nativeFonts.sansBold },
  input: { minHeight: 44, borderWidth: 1, borderColor: colors.stone300, borderRadius: 12, paddingHorizontal: 13, paddingVertical: 10, color: colors.espresso, backgroundColor: colors.white, fontSize: 16, fontFamily: nativeFonts.sans }, multiline: { textAlignVertical: "top" },
  empty: { alignItems: "center", padding: 32, gap: 8 }, emptyTitle: { color: colors.espresso, fontSize: 20, fontFamily: nativeFonts.serifBold, textAlign: "center" }, muted: { color: colors.stone600, lineHeight: 21, fontFamily: nativeFonts.sans, textAlign: "center" },
});
